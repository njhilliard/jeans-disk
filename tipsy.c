...
const char *tipsy_get_last_system_error() { return last_system_error; }
...
void tipsy_set_velocity_scale(float vs) { velocity_scale = vs; }

void tipsy_set_mass_scale(float ms) { mass_scale = ms; }

int tipsy_open_file(const char *filename, const char *mode) {
	if (fd) return TIPSY_ALREADY_OPEN;
...
int tipsy_write_header(double time, int ngas, int ndark, int nstar) {
	if (!fd) return TIPSY_WRITE_UNOPENED;
